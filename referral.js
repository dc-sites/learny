const ReferralSystem = {
  userId: null, points: 0, referred: 0, isSpinning: false,
  segments: [
  ...
  init() {
    ...
    this.loadData(); this.setupUI(); this.drawWheel(); this.bindEvents(); this.updateUI();
    window.addEventListener('storage', e => { if (e.key === 'learny_referrals') this.loadData(); this.updateUI(); });
  },
  ...
  loadData() {
    try {
      ...
      this.points = user.points; this.referred = user.referred;
    } catch { this.points = 0; this.referred = 0; }
  },
  ...
  drawWheel() {
    ...
    this.segments.forEach((seg, i) => {
      ...
      ctx.textAlign = 'right'; ctx.fillStyle = '#fff'; ctx.font = 'bold 11px Inter, sans-serif';
      ctx.fillText(seg.text, r - 15, 4); ctx.restore();
    });
    
    ctx.beginPath(); ctx.arc(cx, cy, 22, 0, 2 * Math.PI);
    ctx.fillStyle = '#1a1a2e'; ctx.fill(); ctx.strokeStyle = '#fff'; ctx.lineWidth = 3; ctx.stroke();
  ...
  spinWheel() {
    ...
    this.isSpinning = true; this.points -= 2; this.saveData(); this.updateUI();
    const canvas = document.getElementById('spin-canvas');
    ...
    setTimeout(() => {
      ...
      this.isSpinning = false; this.updateUI();
    }, 4000);
  },
